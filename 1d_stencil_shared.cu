...
#define RADIUS 3
#define GRIDSIZE 17//128
#define BLOCKSIZE 160//256
...
// -------------------------------------------------------
__global__ void stencil_1D(int *in, int *out, long dim, int blocksize){
...
int main(void){
  ...
  printThreadSizes();
  start_timer(&start);
  stencil_1D<<<gridSize,blockSize>>>(d_in, d_out, N, BLOCKSIZE);
  ...
  std::cout << "--------------------------------------------" << std::endl;
  //std::cout << std::string(h_in) << std::endl;
  //std::cout << std::string(h_out) << std::endl;
  //printArray(h_in, N);
  //printArray(h_out, N);
  free(h_in);
  free(h_out);
